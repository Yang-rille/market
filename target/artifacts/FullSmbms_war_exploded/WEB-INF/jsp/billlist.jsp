...
<div class="right">
       ...
       <form method="get" action="${pageContext.request.contextPath }/bill/query">
			...
			<span>会员：</span>
			<select name="queryMemberId">
				<c:if test="${memberList != null }">
				   <option value="0">--请选择--</option>
				   <c:forEach var="member" items="${memberList}">
				   		<option <c:if test="${member.id == queryMemberId }">selected="selected"</c:if>
				   		value="${member.id}">${member.memName}</option>
				   </c:forEach>
				</c:if>
       		</select>
