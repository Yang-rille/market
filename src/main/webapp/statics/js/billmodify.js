...
$(function(){
  ...
  memberId.next().html("*");
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/bill/memberList",//请求的url
			...
			if(data != null){
				...
				for(var i = 0; i < data.length; i++){
					...
					if(pid != null && pid != undefined && data[i].id == pid ){
						options += "<option selected=\"selected\" value=\""+data[i].id+"\" >"+data[i].memName+"</option>";
					}else{
						options += "<option value=\""+data[i].id+"\" >"+data[i].memName+"</option>";
					}
		...
		error:function(data){//当访问时候，404，500 等非200的错误状态码
			validateTip(memberId.next(),{"color":"red"},imgNo+" 获取会员列表error",false);
		}
	});
	/*
	 * 验证
  ...
  memberId.on("focus",function(){
		validateTip(memberId.next(),{"color":"#666666"},"* 请选择会员",false);
	}).on("blur",function(){
		if(memberId.val() != null && memberId.val() !== "" && memberId.val() !== 0){
			validateTip(memberId.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(memberId.next(),{"color":"red"},imgNo+" 会员不能为空，请选择",false);
		}
    ...
    memberId.blur();
		if(productName.attr("validateStatus") == "true" 
			...
			&& memberId.attr("validateStatus") == "true"){
			if(confirm("是否确认提交数据")){
				$("#billForm").submit();
			}
		}
